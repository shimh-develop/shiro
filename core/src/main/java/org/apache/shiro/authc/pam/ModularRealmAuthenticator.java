...
public class ModularRealmAuthenticator extends AbstractAuthenticator {
    ...
    protected AuthenticationInfo doSingleRealmAuthentication(Realm realm, AuthenticationToken token) {
        //s 是否支持该 AuthenticationToken 这里只有一个 Realm 不支持抛错
    ...
    protected AuthenticationInfo doMultiRealmAuthentication(Collection<Realm> realms, AuthenticationToken token) {
        //s 认证策略 默认AtLeastOneSuccessfulStrategy 至少一个成功
        ...
        for (Realm realm : realms) {
            ...
            try {
                //s 在某个的 Realm 的 getAuthenticationInfo 方法调用之前
            ...
            if (realm.supports(token)) {
                ...
                //s 在某个的 Realm 的 getAuthenticationInfo 方法调用之后
    ...
    protected AuthenticationInfo doAuthenticate(AuthenticationToken authenticationToken) throws AuthenticationException {
        //s 校验 Collection<Realm> realms 应用程序必须提供 Realm 否则抛出异常
