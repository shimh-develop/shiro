...
public class DelegatingSubject implements Subject {
    ...
    public void login(AuthenticationToken token) throws AuthenticationException {
        //s 在 Session 中删除 Runas 标识 [RUN_AS_PRINCIPALS_SESSION_KEY]
        clearRunAsIdentitiesInternal();
        //s 调用 SecurityManager 登录
        Subject subject = securityManager.login(this, token);

        //s 下面将 SecurityManager 返回的 Subject 信息复制到当前的 Subject 上
        //s authenticated设置为true 复制principals信息、host、session
        ...
        if (session != null) {
            //s 装饰 Session 当调用 Session.stop 时调用自身 Subject.sessionStopped方法
            this.session = decorate(session);
        } else {
            this.session = null;
        }
