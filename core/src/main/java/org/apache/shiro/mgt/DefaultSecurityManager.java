...
public class DefaultSecurityManager extends SessionsSecurityManager {
    ...
    public Subject createSubject(SubjectContext subjectContext) {
        ...
        //s 复制一个 重新 new 一个 DefaultSubjectContext
        SubjectContext context = copy(subjectContext);

        //ensure that the context has a SecurityManager instance, and if not, add one:
        //s 确保存在一个SecurityManager
        context = ensureSecurityManager(context);

        //Resolve an associated Session (usually based on a referenced session ID), and place it in the context before
        //sending to the SubjectFactory.  The SubjectFactory should not need to know how to acquire sessions as the
        //process is often environment specific - better to shield the SF from these details:
        //s 从传入的 SubjectContext 解析Session：没有的话根据SubjectContext中的SessionId去SessionManager获取
        context = resolveSession(context);

        //Similarly, the SubjectFactory should not require any concept of RememberMe - translate that here first
        //if possible before handing off to the SubjectFactory:
        //s 解析认证的身份信息：用户名等，从Session等获取（下面的save会往Session存放） 如果设置了RememberMeManager（Cookie）根据当前SubjectContext获取
        context = resolvePrincipals(context);

        //s Subject 由 SubjectFactory 创建 默认是 DefaultSubjectFactory
        ...
        //Added in 1.2:
        //s 由 SubjectDAO 将 身份信息 和 是否身份认证标识 保存到 Session 中 默认是 DefaultSubjectDAO
        save(subject);
    ...
    protected SubjectContext resolveSession(SubjectContext context) {
        ...
        try {
            ...
            //the session manager:
            //s 看有没有SessionId，有的话去 SessionManager 去获取可能存在的Session
    ...
    protected SubjectContext resolvePrincipals(SubjectContext context) {
        //s 各个方式 最大努力获取身份信息 也尝试从 Session 获取 key：[DefaultSubjectContext.PRINCIPALS_SESSION_KEY]
        PrincipalCollection principals = context.resolvePrincipals();

        if (isEmpty(principals)) {
            log.trace("No identity (PrincipalCollection) found in the context.  Looking for a remembered identity.");
            //s 从Cookie中获取 RememberMeManager没有默认实现 需自行设置CookieRememberMeManager
